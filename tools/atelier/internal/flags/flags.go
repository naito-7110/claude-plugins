...
//
// feature-flags プリセット(plugins/atelier/adr/feature-flags.md)は
...
//
// canonical レジストリは RegistryPath(.atelier/flags.yaml)に単一定義する
// (配置の基準は documentation プリセットの C 案: atelier が生成し機械が読む
// 運用ファイルは .atelier/)。形式:
//
//	flags:
//	  <name>:                  # 小文字スネークケース(識別子)
...
const RegistryPath = ".atelier/flags.yaml"

// Package cli は atelier コマンドの引数解析とサブコマンド実行を担う。
package cli
...
サブコマンド:
  ...
  flags check   フラグレジストリ(feature-flags プリセット)を検証する
                .atelier/flags.yaml の形式(owner / expires_on / description 必須)と期限を検査する。
  ...
  branch cleanup  マージ済み agent ブランチ(agent/issue-*)を掃除する
                ...
                --root <dir>           リポジトリのルート(既定: カレントディレクトリ)
                --repo <owner/name>    対象リポジトリ(省略時: カレントリポジトリ)
  gate          PreToolUse hook の機械的ゲート(atelier-gate.sh から exec される)
...
func printReport(deps Deps, report board.Report) int {
	code := ExitOK
	if report.StatusOK {
		fmt.Fprintf(deps.Out, "OK: %s/projects/%d の Status は atelier の 6 値です\n",

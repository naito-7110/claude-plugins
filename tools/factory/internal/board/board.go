// Package board は factory の GitHub Projects ボード操作(複製・検証)を提供する。
//
...
// 正準ボードの既定値と検証仕様。
var (
	// StatusOptions は factory の Status 6 値(この順)。
...
func Copy(client GraphQL, opts CopyOptions) (CopyResult, error) {
	...
	description := fmt.Sprintf("factory ボード(正準テンプレート %s/projects/%d から複製)",

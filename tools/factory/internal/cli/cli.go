...
const usage = `使い方: factory <board|issue|pr|docs|flags|mode|tick> <サブコマンド> [オプション]
...
サブコマンド:
  ...
  mode <verb>   unattended 運転の運転状態(このマシンのローカル状態)を管理する
                verb: status / auto / manual / pause / resume / gate
                状態は .agents/ 配下のローカルファイル(コミットしない)。既定 = manual
                (状態ファイル欠落時も manual — fail-closed)
                gate は night の判定入口: auto かつ非 pause のときだけ exit 0
...
func Run(args []string, deps Deps) int {
	...
	case "mode status", "mode auto", "mode manual", "mode pause", "mode resume", "mode gate":

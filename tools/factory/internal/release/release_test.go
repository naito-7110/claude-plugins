...
func TestRunTagsRemoteDefaultBranchSHA(t *testing.T) {
	...
	out, err := run(t, git, release.Options{Tag: "factory/v0.3.0"})
	if err != nil {
		t.Fatal(err)
	}
	// AC 1: 対象 SHA は常にリモートの実態(RemoteRefs)から解決される。
	// fake はローカル HEAD を持たないため、ローカルの状態は結果に影響しようがない。
	if git.CreatedTags["factory/v0.3.0"] != "abc1234deadbeef" {
		t.Errorf("CreatedTags = %v(origin/main の SHA に打たれるべき)", git.CreatedTags)
	}
	if len(git.PushedTags) != 1 || git.PushedTags[0] != "origin/factory/v0.3.0" {
	...
	for _, want := range []string{
		...
		"タグ factory/v0.3.0 を abc1234deadbeef に作成し、origin へ push しました",
		"リリース workflow(factory-release)が起動します",
	} {
...
func TestRunNonFactoryTagOmitsWorkflowHint(t *testing.T) {
	git := newFakeGit()
	...
	if strings.Contains(out, "factory-release") {
		t.Errorf("factory/v* でないのに workflow ヒントが出ている: %q", out)
	}
}
...
func TestRunExistingLocalTagFails(t *testing.T) {
	git := newFakeGit()
	git.LocalTags["factory/v0.3.0"] = true

	_, err := run(t, git, release.Options{Tag: "factory/v0.3.0"})
...
func TestRunExistingRemoteTagFails(t *testing.T) {
	git := newFakeGit()
	git.RemoteTags["factory/v0.3.0"] = true
...
func TestRunDryRunChangesNothing(t *testing.T) {
	...
	out, err := run(t, git, release.Options{Tag: "factory/v0.3.0", DryRun: true})
	if err != nil {
		t.Fatal(err)
	}
	if len(git.CreatedTags) != 0 || len(git.PushedTags) != 0 {
...
func TestRunDryRunStillDetectsExistingTag(t *testing.T) {
	...
	_, err := run(t, git, release.Options{Tag: "factory/v0.3.0", DryRun: true})

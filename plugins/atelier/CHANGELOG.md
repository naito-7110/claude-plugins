...
ゲートの誤爆解消を中心とした改善リリース(破壊的変更なし)。

- **ゲートのトークン化**(#135): 機械ゲートの判定を「コマンド文字列全体への正規表現部分一致」から「構文トークン化 → 実コマンド識別」へ刷新。コミットメッセージ内の語・連結コマンドの引数混同・引用符すり抜けによる誤爆を解消し、push は refspec(宛先ブランチ)で判定するため worktree からの操作も正しく扱う。判定はトップレベルのコマンド列のみで、ネストしたコマンド(bash -c の中身・コマンド置換)は意図的に対象外(ADR 0002)
- **移行期フォールバックの削除**(#131): issue-check workflow が初回 atelier リリースまで旧 factory バイナリで検証していた暫定措置を撤去(atelier/v リリース済みのため不要)
- **ローカル ADR 0002「配布物の責任境界」**(#134): 今日の 2 事故(crontab 破綻・release 撤去)を 5 whys で「成果物のアイデンティティが持ってよい副作用と配ってよい能力の範囲を決める」に束ね、2 決定(外部状態を持たない / 共有認証下で危険な引き金を配らない)を憲法化

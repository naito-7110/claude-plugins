...
- **観点別のレビューサブエージェントを並列起動する**(Task tool)。各レビュアには **diff と当該観点のプリセットだけ**を渡し、実装の経緯・実装セッションのコンテキストを共有しない(思い込みを知らない新鮮な目で見せる):
  ...
  - **設計整合**(diff がドメイン・契約・公開面に触れる場合のみ — 選択読みと同条件で過剰起動を避ける): domain-modeling(不正状態を公開経路から生成できないか)・design-by-contract(要件 → 契約 → テストの追跡と失敗時状態)・dependency-boundaries(interface パートへ実装詳細が漏出していないか)

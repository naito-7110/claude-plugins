...
> **ガードレールは opt-in・縮退可。** 運用ラベル・Projects ボード・gate バイナリ・required CI が未設置でも本スキルは動く — **あれば**状態管理・強制に使い、**無ければ** issue コメントと規律の自律遵守で代替する。ラベルが未定義なら付与/剥がしは issue コメントの宣言に、ボードが無ければラベルに、と一段ずつ縮退する。価値の本体は憲法(選択読み)と TDD・影響調査・セルフレビューのプロセスにあり、機械強制はその補助。
...
- `agent-ok` あり・`needs-human` なし・`agent-wip` なし(他の作業と衝突しない)。**ラベルが未設置の repo では**着手可否は issue 本文・コメント(groom の「確定済みの設計」節・着手宣言)から判断し、レーンの衝突は open PR / worktree の実態で見る(縮退)
...
gh issue edit <n> --add-label agent-wip   # agent-wip が定義済みなら。未定義なら次行で代替
```

`agent-wip` ラベルが未定義の repo では、代わりに **issue コメントで着手を宣言**して排他を可視化する(縮退)。Projects ボードがあれば Status → In Progress。以降の作業はすべて worktree 内で行う(git-workflow: main へ直接触らない・追従は rebase)。
...
- **検証は /atelier:verify を呼ぶ**(対象は現在の diff)。スタック事実から導出したビルド・テスト・lint を実行し検証契約で返る。verify は直さない — **失敗の自己修正は本スキルの責務**で、同一の失敗クラスにつき **2 回まで**(2 回直しても red ならエスカレーション)。verify が `stack_fact_gaps` を返す・`blocked` の場合は「スタック事実の更新提案」としてエスカレーション(/atelier:init の再実行を提案)
- **diff が依存宣言・ロックファイル・CI の外部参照に触れる場合は /atelier:deps を呼ぶ**。`escalations` / `merge_policy_flags` があれば PR 前に解消するか、人間承認が要るもの(クールダウン例外・許可リスト外ライセンス・脆弱性)はエスカレーションする(supply-chain-security・dependency-licensing)
...
gh issue edit <n> --remove-label agent-wip   # 付与していた場合のみ。未定義なら着手宣言コメントに完了を追記
```
...
- **`merge:agent` なし(既定。ラベル未設置の repo も常にこのレーン)**: ここで終了。人間のレビュー・マージを待つ
...
3. ラベルを付け替える: `gh issue edit <n> --remove-label agent-wip --add-label needs-human`。失格条件(敏感領域・破壊的変更・明記なき依存)に該当した場合は `merge:agent` も外す。**ラベル未設置の repo では** issue コメントに「人間判断待ち(needs-human 相当)」と明示して代替する(縮退)

...
> **ガードレールは opt-in・縮退可。** 本スキルの価値の本体は「1 論点ずつ人間と確定し、確定済みの設計として本文へ書き戻す」プロセスで、これは**ラベル・ボード無しでも完全に成立する**。運用ラベルが未設置なら Ready 化・`merge:agent` 承認は本文の「確定済みの設計」節と issue コメントで表す(下記各手順に縮退を明記)。ボードが無ければラベルに、ラベルも無ければ本文/コメントに、と一段ずつ縮退する。
...
コードベース解析の結果、変更の影響が `.atelier/ownership.yml` 上の **2 ドメイン以上に跨る見込み**なら、先に /atelier:huddle(ドメインエキスパート協議)を実施して「ドメイン別影響分析」を得てから本スキルに戻る(単一ドメインならそのまま進む)。**`ownership.yml` が無い(ドメイン未分割の)repo ではこの判定をスキップし、単一レーンで進む**(縮退)。

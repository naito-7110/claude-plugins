...
> **ガードレールは opt-in・縮退可。** 状態機械の実体は「台帳 + 実態(worktree/PR)」であり、ボード・ラベルはその表現形式にすぎない(本スキルは元々「台帳は参考、実態が真実」)。ボードが無ければラベルで、ラベルも無ければ台帳と実態(open PR・worktree・issue コメント)で状態を復元・配車する。`atelier branch cleanup` が無ければ `gh pr list --state merged` + `git worktree list` で手動回収する。

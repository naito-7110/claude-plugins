...
工場の設置スキル。対象リポジトリに atelier の前提物(ラベル・憲法・スタック事実・`.agents/`)を揃えます。

**冪等**です。再実行しても既存の ADR・CLAUDE.md の記述を壊しません。**対話専用**です(憲法の生成に人間の判断が要るため)。
...
**付与は grooming の場に限定する**: AI が merge-policy(プリセット)を基に「このタスクは agent マージ可か」を提案し、人間が承認して **Ready 化と同時に**付与する。triage は提案コメントまで(ラベルは付けない)。これにより「人間がレビューしていない merge:agent」は構造的に存在しない。work は着手時に**鮮度チェック**(付与後に issue 本文・受け入れ条件が変わっていないか)を行い、変わっていれば agent マージせず人間レーンへ降格する。
...
```bash
# 正準ボード(atelier が公開するテンプレート)からのコピーを優先。
...
- 開発判断の根拠は二層: atelier プラグイン同梱のプリセット ADR(ポータブル原則。一覧はプラグインの `adr/README.md`)+ このリポジトリの `docs/adr/`(プロジェクト固有の決定。技術選定はこちら)
...
**atelier バイナリ(`.agents/bin/`)— 必ず `.atelier/` scaffold より先に取得する**:
...
**atelier 運用ファイル(`.atelier/`)** — 配置の基準は「誰の持ち物か」(documentation プリセット): atelier が生成し機械が読む運用ファイルは `.atelier/`(dotdir、既存リポジトリの慣習と競合しない)、人間の一次文書(ADR・ドメイン知識)は標準の `docs/` に置く。無ければ作成する(既存は壊さない)。**`.atelier/` はコミット対象**(gitignore しない)。**`.atelier/` を作った時点でこのリポジトリは hook ゲートの対象になる**(バイナリ取得を先に済ませていること):

- `.atelier/README.md`: 文書の地図。各層(プリセット + `docs/adr/` / `docs/domains/` / CLAUDE.md マーカー節 / `.atelier/`)の場所と役割の案内
- `.atelier/ownership.yml`: 機械可読な所有マップ。`atelier docs verify` が検証する:
...
**ドメインの定義・分割は init では行わない**。`domains: {}` の空マップを置くまでが init の仕事で、ドメインを切る意思決定と `docs/domains/` 雛形の生成は /atelier:domains(専用スキル)が対話で行う(分割基準は domain-partitioning プリセット)。
...
- `.github/workflows/atelier-issue-check.yml`(PR↔issue 整合のサーバー側ゲート。`templates/workflows/` から)
...
既にコードのあるリポジトリでは、決定はコードに暗黙で存在し、文書は「ある場合」も「ない場合」も「あるが古い場合」もある。**init がやるのは棚卸し・突き合わせ・起票まで。生成・是正の本体は issue 化して atelier の通常レーン(triage → groom → work)で処理する**(監査は網羅、対処は漸進):

1. **棚卸し**: 既存文書を網羅的に列挙する — README・docs/ 配下・別形式の ADR・設計メモ・コード内の規約コメント。大きいリポジトリでは観点別サブエージェントに並列で読ませてよい
2. **突き合わせ**: 各文書をコードと照合して 3 分類する
   - **正しい** → `.atelier/README.md`(地図)に登載し、`ownership.yml` の所有に含める(以後は documentation プリセットの「同 PR 更新」が腐敗を防ぐ)
   - **古い / コードと矛盾** → 是正 issue を起票してレーンに乗せる(init 中に直さない)
   - **決定の記録に相当する内容** → 下記 3 の記録 ADR へ蒸留する候補に加える
3. **暗黙決定の抽出と記録 ADR 化**: コード解析で暗黙の決定候補を抽出する。優先するのは、プリセットが「プロジェクト判断」に委ねている点: 技術選定(フレームワーク・ORM・主要ライブラリ)・レイヤ構成・エラー処理の型・認可の置き場所・命名 / 配置規約。候補を一覧提示し、**人間が選んだものだけ**を「記録 ADR」として `docs/adr/` に起こす(冒頭に**「遡及記録: 既に採用済みの決定の文書化であり、新規決定ではない」**を明示。改廃は通常の /atelier:adr で)
4. **欠落の起票**: 文書が無い領域は、atelier の文書モデル(スタック事実 / ローカル ADR / ドメイン文書)に対する欠落として issue 化する。ドメイン文書の生成は /atelier:domains をオンボーディングの続きとして推奨する(当たり = ディレクトリ構成・共変更の 1 段解析だけここで示す)
...
- 実行依存(`jq`・atelier バイナリ)が揃っているかを確認する
...
| CLAUDE.md スタック事実 | ✅ |
| atelier 運用ファイル(.atelier: 地図 + 空の所有マップ) | ✅ / 既存を尊重 |
| issue / PR テンプレート + issue-check workflow(.github) | ✅ / 既存は確認の上 |
| atelier バイナリ(.agents/bin) | ✅ / スキップ(リリース未整備) |
| 後入れ認識(文書監査) | 登載 n / 是正 issue n / 記録 ADR n / スキップ(新規) |
| hook ゲート | 有効(自動登録) / 要確認(依存欠落・新セッション待ち) |
| `.agents/` | ✅ |

旧名 factory(v0.2.x 以前)からの移行の場合: `.factory/` が残っていたら `git mv .factory .atelier` を提案する(管理判定が `.atelier/` になったため、放置するとゲートが働かない)。旧バイナリ `.agents/bin/factory` は削除してよい。

残る手動作業(提示のみ。実行しない):

- Projects ボードの Status 選択肢変更・auto-add(Web UI)
- **atelier-issue-check を required status check に登録**(全クライアントを縛る L3): `gh api -X PUT "repos/{owner}/{repo}/branches/main/protection" --input -` で `required_status_checks.contexts` に `atelier-issue-check` を含める(既存の保護設定とマージすること)。**agent マージを使う場合は `atelier-review` も追加**(レビュア green なしのマージをサーバー側でも遮断)
- **GitHub の「Automatically delete head branches」を有効化**(Settings → General): マージ済みリモートブランチの掃除は GitHub に委ね、ローカルは `atelier branch cleanup` が担う
- ブランチ保護(マージゲートの機械的強制)

最後に、最初の issue の起票と /atelier:groom での仕様揉みを提案する。

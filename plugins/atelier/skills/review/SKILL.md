---
name: review
description: 別コンテキストレビュア(人間が起動する)。レビュー待ちの agent PR を自分で検出し、diff と憲法だけを材料に独立レビューして commit status(atelier-review)で判定を表明する。merge:agent の実行条件「別コンテキストレビュア green」の実体
tools:
  - Bash(gh, git, atelier)
  - Task
  - Read

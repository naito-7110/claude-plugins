...
3. **相互チェック段 — 突き合わせ(huddle 同型)**: 全レンズの指摘と PM の受け入れ条件照合を突き合わせ、次を機械的に検出する:
   ...
   - **観点間の矛盾**: あるレンズが許容・推奨した設計を別レンズが問題視(例: granularity「この PR で完結」↔ design「契約変更は別 PR」/ tests「状態検証で足りる」↔ contract「失敗時状態のテストが欠落」)
   - **カバレッジの隙間**: 受け入れ条件・diff 領域のうち、どのレンズの観点でも評価されていない箇所(選択読みで引くべき preset の漏れ・レンズ未カバー領域)
4. **第 2 相 — 収束**: 矛盾ごとに、当事者レンズへ**相手の結論サマリだけ**を渡して再評価を求める(渡すのは他レンズの結論であって実装ジャーナル・実装コンテキストではない — 独立性は保つ)。隙間は該当レンズ / preset を追加 dispatch して埋める。PM が収束を判定し、**収束しない矛盾は捏造せず両論併記にする**(どの観点でどう割れたか・各々の pros/cons を残し、人間の裁定材料にする)

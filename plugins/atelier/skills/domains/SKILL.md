...
- **共変更の分析**: /atelier:analyze の影響マップ(`co_change`)で「同じ PR / コミットで一緒に変わるファイル群」を得る(変更理由の単位の実測)
...
- `.atelier/ownership.yml` に `domains.<name>.paths` を追記(既存の宣言は壊さない)
...
- `atelier docs verify` が通らない状態での PR 作成

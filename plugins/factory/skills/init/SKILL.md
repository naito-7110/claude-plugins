...
### 3. Projects ボード(任意・半自動)

`project` スコープがあれば(`gh auth status` で確認)、ボード作成を提案する。なければ `gh auth refresh -s project` を案内するが、**ラベルのみでも全スキルは動作する**のでスキップしてよい。
...
**ガード**: 技術選定(「frontend は Vue で組む」「ORM は Prisma」の類)が挙がったら ADR には書かず、スタック事実(CLAUDE.md)行きであることを説明する。ADR はスタック非依存の原則に限定し、プロジェクトを跨いでポータブルに保つ。
...
> **書いてはいけないこと**: 技術選定(フレームワーク・ライブラリ・言語の指名)。それは CLAUDE.md のスタック事実へ。

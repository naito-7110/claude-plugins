...
- **factory-issue-check を required status check に登録**(全クライアントを縛る L3。unattended 運転の前提条件): `gh api -X PUT "repos/{owner}/{repo}/branches/main/protection" --input -` で `required_status_checks.contexts` に `factory-issue-check` を含める(既存の保護設定とマージすること)

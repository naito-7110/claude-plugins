...
### 6.4 後入れ認識(既存コードがあるリポジトリのみ・対話)

既にコードのあるリポジトリでは、「既になされた決定」がコードの中に暗黙で存在する。local ADR が空のまま無人の work が走ると既存規約と矛盾する実装をしうるため、読める形に起こす:

1. **コードベースを解析し、暗黙の決定候補を抽出する**。優先するのは、プリセットが「プロジェクト判断」に委ねている点: 技術選定(フレームワーク・ORM・主要ライブラリ)・レイヤ構成・エラー処理の型・認可の置き場所・命名 / 配置規約
2. 候補を一覧提示し、**人間が選んだものだけ**を「記録 ADR」として `docs/adr/` に起こす。様式は通常の ADR と同じだが、冒頭に**「遡及記録: 既に採用済みの決定の文書化であり、新規決定ではない」**を明示する(改廃したくなったら通常の /factory:adr で)
3. ドメイン構造は当たり(ディレクトリ構成・共変更の 1 段解析)だけ示し、本格的な分割は /factory:domains を案内する
4. **`.agents/` には現状認識をシードしない** — 台帳は orchestrate が GitHub と worktree の実態から毎回再構築する設計であり、キャッシュに真実を持たせない
...
| 後入れ認識(記録 ADR) | n 本作成 / スキップ(新規リポジトリ) |
...
- **cron の登録**(unattended 運転を使う場合): `flock` による多重起動防止つきで `claude -p "/factory:night"` を定期起動する(crontab 例は night スキル参照)

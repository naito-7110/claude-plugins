...
- diff 全体を、選択読みした憲法に照らして確認する(禁止事項・敏感領域・粒度)
- 対象リポジトリに専用レビューエージェント定義(`.claude/agents/`)があれば委譲してよい。指摘(critical / major)には対応し、内容をジャーナルへ記録する
- **セルフレビューは merge:agent の実行条件を満たさない**(別コンテキストレビュアが必要 — merge-policy)

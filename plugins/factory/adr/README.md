...
**常時読み**(タスクの種類によらず): spec-alignment・pr-granularity・merge-policy・git-workflow、および product-security の「エージェントの裁量制限」節(敏感領域判定)。
...
| DB・スキーマ・排他 | schema-migration・concurrency-process・tdd-doctrine |
...
複数領域に跨る場合は和集合。迷ったら読む側に倒す(fail-closed)。第 2 弾プリセット(schema-migration・dependency-licensing・data-lifecycle・ai-usage)は #41 の PR がマージされ次第有効。
